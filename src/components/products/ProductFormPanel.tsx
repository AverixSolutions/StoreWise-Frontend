// src/components/products/ProductFormPanel.tsx
"use client";
...
export default function ProductFormPanel({
  ...
  }, [editProduct, initialTab, isEmbedded]);
  ...
  const panel = (
    <div
      className={
        isEmbedded
          ? "h-full max-h-full min-h-0 w-full rounded-[24px] border border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.98))] backdrop-blur overflow-hidden flex flex-col"
          : "min-h-0 w-full sm:max-w-2xl sm:rounded-[24px] rounded-t-[24px] bg-[linear-gradient(180deg,rgba(255,255,255,0.97),rgba(248,250,252,0.97))] shadow-[0_-10px_60px_rgba(3,10,24,0.18)] backdrop-blur overflow-hidden flex flex-col max-h-[92dvh] sm:max-h-[88dvh]"
      }
    ...
    >
      ...
      {activeTab === "single" ? (
        <form
          ...
          className="min-h-0 flex-1 overflow-y-auto scroll-pt-24 scroll-pb-36 px-4 py-3 sm:px-5 sm:py-3 space-y-3 no-scrollbar"
        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <SearchableDropdown
                      ...
                      options={subcategoryOptionsForTypedParent.map((row) => ({
                        value: row.name,
                        label: row.name,
                      }))}
                      placeholder={
          ...
          {/* ── Compact Row: Item Name + Image ── */}
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_112px] sm:items-start">
            <div>
            ...
            <div>
              ...
              <button
                ...
                className="group flex h-[86px] w-full items-center justify-center overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-slate-50 transition hover:border-cyan-300 hover:bg-cyan-50/40"
              >
                {imagePreviewUrl ? (
                  <img
                    src={imagePreviewUrl}
        ...
        /* ── Bulk tab ── */
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5 sm:py-4 space-y-4 no-scrollbar">
