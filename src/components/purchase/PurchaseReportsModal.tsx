// src/components/purchase/PurchaseReportsModal.tsx
"use client";
...
export interface PurchaseReportsModalProps {
  isOpen: boolean;
  onClose: () => void;
  licenseId: string;
...
export default function PurchaseReportsModal({
  isOpen,
  onClose,
  licenseId,
  suppliers,
  ...
  async function handleDelete(id: string) {
    ...
    if (!ok) return;
    await (window as any).electronAPI.deletePurchase(id);
    refresh();
  }

  function openRow(row: Row) {
    onOpenPurchase(row.id);
  }
    ...
    <div className="fixed inset-0 z-[1000] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden border border-gray-200">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-gray-50 to-white">
          <div className="text-base font-semibold">Purchase Reports</div>
          <button
            onClick={onClose}
        ...
        {openingId && (
          <div className="px-6 py-2 text-sm bg-amber-50 text-amber-800 border-b border-amber-200">
            Opening purchase <span className="font-medium">{openingId}</span>…
          </div>
        )}
