...
export default function ReturnHoldsModal({
  ...
  async function refresh() {
    setLoading(true);
    try {
      const res = await (window as any).electronAPI.listPurchaseReturnHolds(
        licenseId,
        { page: 1, pageSize: 200 }
      );
      setHolds(res.holds || []);
  ...
  async function handleDelete(id: string) {
    ...
    if (!ok) return;
    await (window as any).electronAPI.deletePurchaseReturnHold(id);
    refresh();
  }
  ...
  async function confirmRename(newTitle: string) {
    if (!renameId) return;
    await (window as any).electronAPI.savePurchaseReturnHold({
      id: renameId,
      title: newTitle || null,
    });
    setRenameId(null);
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="flex items-center gap-3 text-gray-500">
                <div className="w-5 h-5 border-2 border-averix-red-vivid/20 border-top-averix-red-vivid rounded-full animate-spin"></div>
