// src/components/purchase-return/ReturnHoldsModal.tsx
"use client";
...
interface ReturnHoldsModalProps {
  isOpen: boolean;
  onClose: () => void;
  licenseId: string;
...
export default function ReturnHoldsModal({
  isOpen,
  onClose,
  licenseId,
  onResume,
}: ReturnHoldsModalProps) {
  ...
  async function refresh() {
    setLoading(true);
    try {
      const res = await platform.listPurchaseReturnHolds?.(licenseId, {
        page: 1,
        pageSize: 200,
      });
      setHolds(res?.holds || []);
  ...
  async function confirmRename(newTitle: string) {
    if (!renameId) return;
    try {
      // Fetch the current hold to get its full data
      const holdRes = await platform.getPurchaseReturnHold?.(renameId);
      if (!holdRes?.success || !holdRes.hold) return;

      const { header, rows } = holdRes.hold;
      await platform.savePurchaseReturnHold?.({
        id: renameId,
        licenseId,
        header,
        rows,
      ...
      console.error("Failed to rename hold", err);
    }
  }

  if (!isOpen) return null;

  // JSX identical to previous answer — no changes needed here
  return (
    <div
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs" style={{ color: "#8ea3c7" }}>
                Manage your saved return states
              </div>
            </div>
          </div>
          <button
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-2">
              ...
              <span
                ...
                style={{ color: "#8ea3c7" }}
              >
                Create holds to save your purchase return progress and resume
                later
              </span>
            </div>
          ) : (
