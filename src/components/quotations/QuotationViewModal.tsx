...
export default function QuotationViewModal({
  ...
  async function handlePrint() {
    if (!quotationId) return;
    try {
      await printQuotation(quotationId);
    } catch (err: any) {
  ...
  }, [isOpen, onClose, quotationId, showSettings]);
            ...
            <div className="shrink-0 border-t border-slate-100 px-5 py-3 flex items-center justify-between gap-3 bg-white">
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={handlePrint}
                  className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm text-slate-600 transition hover:bg-slate-100 cursor-pointer"
                >
                  <Printer className="w-4 h-4" />
                  Print
                  <span className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 font-mono text-[9px] text-slate-400">
                    Ctrl+P
                  </span>
                </button>
                <button
