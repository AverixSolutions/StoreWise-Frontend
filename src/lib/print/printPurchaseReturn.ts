...
} from "./buildPurchaseReturnInvoiceHtml";
import { buildPurchaseReturnThermalHtml } from "./buildPurchaseReturnThermalHtml";
import { getTaskPref, type PaperSize } from "./printPreferences";
import { getPurchaseReturnPrintCustomization } from "./purchaseReturnPrintCustomization";

type ShowToast = (type: "success" | "error" | "info", message: string) => void;

type PurchaseReturnPrintOverrides = {
  preview?: boolean;
  silent?: boolean;
  paperSize?: PaperSize;
};
...
async function loadSupplierDetails(
  ...
  try {
    ...
    console.warn("[print-return] supplier details unavailable", error);
    return null;
  }
}
...
async function resolveTransactionType(
  ...
  const direct =
    clean(purchaseReturn.transactionTypeName) ||
    clean(purchaseReturn.transactionType) ||
    clean(purchaseReturn.typeName);

  if (direct) return direct;

  const typeId = clean(purchaseReturn.typeId);
  if (!typeId || !platform.listTransactionTypes) return null;

  try {
    const result = await platform.listTransactionTypes(
      licenseId,
      "purchaseReturn",
    );
    ...
    console.warn("[print-return] transaction type unavailable", error);
    return null;
  }
}

export async function printPurchaseReturn(
  returnId: string,
  overrides?: PurchaseReturnPrintOverrides,
  showToast?: ShowToast,
) {
  const isDesktop = !!(window as any).electronAPI;
  const pref = getTaskPref("purchaseReturn");
  const options = getPurchaseReturnPrintCustomization();
  ...
  if (isDesktop) {
    ...
    if (!api?.getPurchaseReturnFull) {
      throw new Error("Purchase Return print API not available");
    }
    response = await api.getPurchaseReturnFull(returnId);
  } else {
    response = await platform.getPurchaseReturnFull?.(returnId);
  }

  if (!response?.success) {
    throw new Error(response?.error || "Failed to load Purchase Return");
  }

  const purchaseReturn = response.purchaseReturn;
  const items = response.items || [];
  if (!purchaseReturn) {
    throw new Error("Purchase Return data missing");
  }

  const licenseId =
    clean(purchaseReturn.licenseId) ||
    clean(localStorage.getItem("licenseId")) ||
    "demo-license";

  const [shop, supplier, transactionType] = await Promise.all([
    getShopProfile(licenseId),
    loadSupplierDetails(purchaseReturn.supplierId),
    resolveTransactionType(licenseId, purchaseReturn),
  ]);
  ...
  const discount = Number(purchaseReturn.discount || 0);
  const grandTotal = Math.max(0, subTotal - discount);

  const printInput: PurchaseReturnPrintInput = {
    shop,
    options,
    bill: {
      entryNo: purchaseReturn.slNo,
      billNo: purchaseReturn.billNo,
      date: purchaseReturn.returnDate,
      time: purchaseReturn.entryTime,
      supplierName:
        clean(purchaseReturn.supplierName) ||
        clean(supplier?.name) ||
      ...
      debitAccount: purchaseReturn.debitAccount,
      natureOfEntry: purchaseReturn.natureOfEntry,
      purchaseType: purchaseReturn.purchaseType,
      transactionType,
    },
    items: items.map((item: any, index: number) => ({
      ...
      qty: Number(item.appliedQuantity ?? item.quantity ?? 0),
      unit: item.unit,
  ...
  const html = isThermal
    ? buildPurchaseReturnThermalHtml(printInput)
    : buildPurchaseReturnInvoiceHtml(printInput);

  const documentTitle =
    String(options.documentTitle || "").trim() || "Purchase Return";
  const title = `${documentTitle} - ${
    purchaseReturn.billNo ?? purchaseReturn.slNo ?? ""
  }`;
  ...
  if (isDesktop) {
    ...
    if (!usePreview) {
      if (result?.success) {
        showToast?.("success", "Purchase Return printed successfully");
      } else {
        showToast?.(
          "error",
